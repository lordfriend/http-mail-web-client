...
angular.module('ngPasswordComplexify', [])
  .factory('Complexify', function($q) {
    ...
    Complexify.prototype.additionalComplexityForCharset = function (str, charset) {
    ...
    Complexify.prototype.inBanlist = function (str) {
      if (this.options.banMode === 'strict') {
        for (var i = 0; i < banlist.length; i++) {
          if (str.toLowerCase().indexOf(banlist[i].toLowerCase()) !== -1) {
            return true;
          }
        }
        return false;
      } else {
        return banlist.indexOf(str) > -1;
      }
    };

    Complexify.prototype.evaluateSecurity = function (password) {
      ...
      if (!this.inBanlist(password)) {
        ...
        for (var i = CHARSETS.length - 1; i >= 0; i--) {
          complexity += this.additionalComplexityForCharset(password, CHARSETS[i]);
        }
      ...
      if(valid) {
        deferred.resolve(complexity);
      } else {
        deferred.reject(complexity);
      }
  ...
  .directive('ngPasswordComplexify', function(Complexify) {
    return {
      restrict: 'A',
      require: 'ngModel',
      link: function($scope, $element, $attrs, ngModelCtrl) {
        ...
        function validComplexity() {
          complexify.evaluateSecurity(ngModelCtrl.$viewValue)
            .then(function(){
              ngModelCtrl.$setValidity('pw-complexity', true);
            }, function() {
              ngModelCtrl.$setValidity('pw-complexity', false);
            });
          return ngModelCtrl.$viewValue;
        }

        ngModelCtrl.$parsers.unshift(validComplexity);
      }
    };
  })
